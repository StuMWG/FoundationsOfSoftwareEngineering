    ...
    private void drawBoard(Graphics g) {
        ...
        for (int row = 0; row < size; row++) {
            for (int col = 0; col < size; col++) {
                ...
                // Use the default color (e.g., white) if there’s no color assigned
                if (control.getSosSequenceMap().containsKey(point)) {
                    g.setColor(control.getSosSequenceMap().get(point)); // Color from sequence map
                } else {
                    g.setColor(Color.WHITE); // Default color for cleared cells
                }
                ...
                // Draw the symbol (S or O) if present
